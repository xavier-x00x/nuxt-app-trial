...
class PHPJasperXML extends abstractPHPJasperXML
{
         ...
         foreach ($this->arraylastPageFooter as $out) {
            switch ($out["hidden_type"]) {
               ...
               case "field":
                  $this->display(
                     $out,
                     $this->arrayPageSetting["pageHeight"] - $this->arraylastPageFooter[0]["height"] - $this->arrayPageSetting["bottomMargin"],
                     true
                  );
                  break;
               default:
   ...
   public function detail()
   {
      ...
      if ($this->totalgroup > 0) {
         ...
         $this->report_count++;
         $this->global_pointer++;
         $checkpoint = $this->showGroupFooter($totalgroupheight + $this->maxpagey['page_' . ($this->pdf->getNumPages() - 1)], false);
   ...
   public function showGroupFooter($y = 0, $bool = true)
   {
      // return $y;
      ...
      // dwi fix footer
      if (count($this->arraysqltable) == $this->report_count) {
         // echo $this->totalgroup . "<br/>";
         $groupnox = $this->totalgroup - 1;
         for ($groupnox; $groupnox  >= 0; $groupnox--) {
            // echo $groupnox . ' name : ' . print_r($this->grouplist[$groupnox]['name'], true) . "<br/>";
            $bandheight = $this->grouplist[$groupnox]['groupfootheight'];
            $footercontentx = $this->grouplist[$groupnox]["footercontent"];
            foreach ($footercontentx as $i => $out) {
               $this->display($out, $y, true);
            }
            $y = $y + $bandheight;
         }
