...
class PHPJasperXMLElement extends abstractPHPJasperXML
{
    ...
    public function element_staticText($data, $elementid)
    {
        ...
        // BY DWI
        if (isset($data["textAdjust"]) && $data["textAdjust"] == "StretchHeight") {
            $stretchoverflow = "true";
        }
    ...
    public function element_rectangle($data, $elementid)
    {
        ...
        $border = array("LTRB" => array(
            'width' => $borderwidth + 0, 'color' => $drawcolor, 'cap' => 'square',
        ...
        switch ($data->textFieldExpression) {
            ...
            case '$V{PAGE_NUMBER}':
                ...
                if (isset($data["evaluationTime"]) && $data["evaluationTime"] == "Report") {
                    $mydata[] = array(
                        "type" => "MultiCell", "width" => $data->reportElement["width"], "height" => $height, "txt" => '{{:ptp:}}', "border" => $border, "align" => $align, "fill" => $fill, "hidden_type" => "pageno", "soverflow" => $stretchoverflow, "poverflow" => $printoverflow, "link" => $data->hyperlinkReferenceExpression,
                    ...
                    $mydata[] = array(
                        "type" => "MultiCell", "width" => $data->reportElement["width"], "height" => $height, "txt" => '$this->PageNo()', "border" => $border, "align" => $align, "fill" => $fill, "hidden_type" => "pageno", "soverflow" => $stretchoverflow, "poverflow" => $printoverflow,
                        "font" => $font, "pdfFontName" => $pdffont, "fontstyle" => $fontstyle . "", "fontsize" => $fontsize + 0,
